...
async def test_writes_file(tmp_path):
    ...
    ctx = PipelineContext("p", "p/2026-04-28", "2026-04-28", [object()], {"markdown": "# hi"})
...
async def test_overwrite_on_retry(tmp_path):
    ...
    s = FileMdSink(id="local", input="context.markdown", path=str(out), mode="upsert")
    async with open_db(tmp_path / "d.db") as db:
        e = EmissionStore(db)
        await s.emit(PipelineContext("p", "p/r", "r", [1], {"markdown": "v1"}), emissions=e)
        await s.emit(PipelineContext("p", "p/r", "r", [1], {"markdown": "v2"}), emissions=e)
...
async def test_path_traversal_rejected(tmp_path):
    ...
    s = FileMdSink(id="local", input="context.markdown", path=str(out))
    ctx = PipelineContext(
        "../../../etc/passwd", "p/2026-04-28", "2026-04-28", [object()], {"markdown": "# hi"}
    )
    async with open_db(tmp_path / "d.db") as db:
        e = EmissionStore(db)
        res = await s.emit(ctx, emissions=e)
