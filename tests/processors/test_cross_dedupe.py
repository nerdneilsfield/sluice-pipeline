...
@pytest.mark.asyncio
async def test_title_dedup_merges_tags_case_sensitively_in_order():
    a = make_item(url="https://x.com/a", title="GCC 16 released", tags=["compiler"])
    b = make_item(url="https://x.com/b", title="GCC 16 released!", tags=["compiler", "release"])

    ctx = await _proc(merge_tags=True, title_similarity_threshold=0.85).process(
        make_ctx(items=[a, b])
    )

    assert ctx.items[0].tags == ["compiler", "release"]
...
async def test_debug_logs_each_dropped_item_with_merge_details(monkeypatch):
    ...
    assert merge_events[0]["ratio"] == 1.0
...
def test_threshold_out_of_range_raises():
    ...
    with pytest.raises(ConfigError):
        CrossDedupeConfig(type="cross_dedupe", name="x", title_similarity_threshold=-0.1)
