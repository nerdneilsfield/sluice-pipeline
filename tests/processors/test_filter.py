...
async def test_keep_if_all():
    items = [mk(summary="x" * 100), mk(summary="short"), mk(summary="x" * 200)]
    p = FilterProcessor(
        name="f", mode="keep_if_all", rules=[FilterRule(field="summary", op="min_length", value=50)]
    )
...
async def test_drop_if_any():
    ...
    p = FilterProcessor(
        name="f",
        mode="drop_if_any",
        rules=[FilterRule(field="title", op="matches", value=r"(?i)\bad\b")],
    )
    out = await p.process(ctx_with(items))
...
async def test_numeric_gte():
    ...
    b.extras["score"] = 3
    p = FilterProcessor(
        name="f", mode="keep_if_all", rules=[FilterRule(field="extras.score", op="gte", value=5)]
    )
...
async def test_newer_than():
    ...
    p = FilterProcessor(
        name="f",
        mode="keep_if_all",
        rules=[FilterRule(field="published_at", op="newer_than", value="48h")],
    )
...
def test_redos_pattern_times_out():
    """Catastrophic backtracking like (a|aa)+$ must be rejected by timeout."""
    from sluice.processors.filter import _safe_search

    with pytest.raises(ValueError, match="regex timeout"):
        _safe_search(r"(a|aa)+$", "a" * 500 + "b")
...
async def test_eq():
    ...
    p = FilterProcessor(
        name="f", mode="keep_if_all", rules=[FilterRule(field="title", op="eq", value="a")]
    )
    out = await p.process(ctx_with(items))
...
async def test_not_in():
    items = [mk(title="a"), mk(title="b")]
    p = FilterProcessor(
        name="f", mode="keep_if_all", rules=[FilterRule(field="title", op="not_in", value=["a"])]
    )
    out = await p.process(ctx_with(items))
...
async def test_max_length():
    ...
    p = FilterProcessor(
        name="f", mode="keep_if_all", rules=[FilterRule(field="summary", op="max_length", value=50)]
    )
    out = await p.process(ctx_with(items))
    assert len(out.items) == 1
...
async def test_exists():
    ...
    p = FilterProcessor(
        name="f", mode="keep_if_all", rules=[FilterRule(field="summary", op="exists")]
    )
    out = await p.process(ctx_with(items))
    assert len(out.items) == 1
