...
def test_run_invokes_runner(tmp_path):
    fake = AsyncMock(
        return_value=type(
            "R", (), {"status": "success", "run_key": "p/r", "items_out": 3, "error": None}
        )()
    )
    with patch("sluice.cli.run_pipeline", fake), patch("sluice.cli.load_all") as load:
...
def test_run_passes_dry_run_flag(tmp_path):
    fake = AsyncMock(
        return_value=type(
            "R", (), {"status": "success", "run_key": "p/r", "items_out": 0, "error": None}
        )()
    )
    with patch("sluice.cli.run_pipeline", fake), patch("sluice.cli.load_all") as load:
        load.return_value = type("B", (), {"pipelines": {"p": object()}})()
