...
| 自托管可观测         | n8n web UI（重）            | `print` + grep          | Prefect server 看每条 pipeline 的 run history          |
...
4 层 LLM 降级、幂等重试、dry-run、Prefect 调度、SSRF 防护。
...
pytest                              # 147 个测试，约 6 秒
pytest --cov=sluice                 # 91% 覆盖率
ruff check .
ty check .
```
