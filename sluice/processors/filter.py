...
def _safe_search(pattern: str, text: str) -> bool:
    ...
    try:
        return bool(regex.search(pattern, text, timeout=1.0))
    except TimeoutError:
...
def _eval(rule: FilterRule, item: Item) -> bool:
    ...
    if op == "eq":
        return v == target
    if op == "matches":
        return bool(_safe_search(target, str(v)))
    if op == "not_matches":
        return not _safe_search(target, str(v))
    if op == "contains":
        return target in v
    if op == "not_contains":
...
class FilterProcessor:
    ...
    def _matches(self, item: Item) -> bool:
        ...
        if self.mode == "keep_if_all":
            return all(results)
        if self.mode == "keep_if_any":
            return any(results)
        if self.mode == "drop_if_all":
            return not all(results)
        if self.mode == "drop_if_any":
